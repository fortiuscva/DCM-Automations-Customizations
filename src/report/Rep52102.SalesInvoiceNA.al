report 52102 "THK Sales Invoice NA"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/THKSalesInvoiceNA.rdl';
    Caption = 'THK Sales - Invoice';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(ShptDate_SalesInvHeader; Format("Sales Invoice Header"."Shipment Date", 0, 4))
                    {
                    }
                    ...
                    column(DocumentDate_SalesInvHeader; Format("Sales Invoice Header"."Document Date", 0, 4))
                    {
                    }
                    ...
                    column(CompanyHomePageCaptionLbl; HomePageLbl)
                    {
                    }
                    ...
                    column(BillToAddress_Lbl; BillToLbl)
                    {
                    }
                    ...
                    column(ShippedFromDetails1; ShippedFromDetails[1])
                    {
                    }
                    column(ShippedFromDetails2; ShippedFromDetails[2])
                    {
                    }
                    column(ShippedFromDetails3; ShippedFromDetails[3])
                    {
                    }
                    column(ShippedFromDetails4; ShippedFromDetails[4])
                    {
                    }
                    column(ShippedFromDetails5; ShippedFromDetails[5])
                    {
                    }
                    column(ShippedFromDetails6; ShippedFromDetails[6])
                    {
                    }
                    column(ShippedFromDetails7; ShippedFromDetails[7])
                    {
                    }
                    column(ShippedFromDetails8; ShippedFromDetails[8])
                    {
                    }
                    column(ShippedFromDetails9; ShippedFromDetails[9])
                    {
                    }
                    column(BillToCustomerNoCaption; BillToCustomerNoCaptionLbl)
                    {
                    }
                    column(SalesInvHead_BillToCust; "Sales Invoice Header"."Bill-to Customer No.")
                    {
                    }
                    column(ShippingAgentcodeCaption; ShippingAgentcodeCaptionLbl)
                    {
                    }
                    column(SalesInvHead_ShippingAgentCode; ShippingAgentRecGbl.Name)
                    {
                    }
                    column(CustomerVATRegNoCaption; CustomerVATRegNoCaptionLbl)
                    {
                    }
                    column(SalesInvHead_VATRegNo; "Sales Invoice Header"."VAT Registration No.")
                    {
                    }
                    column(EORINoCaption; EORINoCaptionLbl)
                    {
                    }
                    column(EORINo; Customer."EORI Number")
                    {
                    }
                    ...
                    dataitem(SalesInvLine; "Integer")
                    {
                        ...
                        column(ItemNo_Line; TempSalesInvoiceLine."No.")
                        {
                        }
                        column(ItemNo_Line_Lbl; TempSalesInvoiceLine.FieldCaption("No."))
                        {
                        }
                        column(ShipmentDate_Line; Format(TempSalesInvoiceLine."Shipment Date"))
                        {
                        }
                        ...
                        column(VATPct_Line_Lbl; TempSalesInvoiceLine.FieldCaption("VAT %"))
                        {
                        }
                        ...
                        begin
                            ...
                            with TempSalesInvoiceLine do begin
                                ...
                                */
                                ...
                                If NewLine and (Type <> Type::" ") and (Quantity = 0)then HideLine:=true end;
                            CollectAsmInformation(TempSalesInvoiceLine);
                            FormatLineValues(TempSalesInvoiceLine);
                        end;
                        trigger OnPreDataItem()
                        begin
                            Clear(TaxLiable);
 ...
 Clear(CopyTxt)
            ...
            begin
                ...
                */
                ...
                BankDetails[1]:='BANK DETAILS:';
                BankDetails[2]:='For EURO: BE50 9670 2459 4718';
                BankDetails[3]:='Bank code (SWIFT / BIC) - TRWIBEB1XXX';
                BankDetails[4]:='For GBP: 64586508 UK Sort Code 23-14-70';
                ShippedFromDetails[1]:='Shipped from Tomahawk Manufacturing B.V. The Netherlands';
                ShippedFromDetails[2]:='Nijverheidsstraat 14-16 2288 BB Rijswijk';
                ShippedFromDetails[3]:='VAT: NL859069515B01                               EORI: NL859069515';
                ShippedFromDetails[4]:='All Goods (Food Processing Machine Parts)';
                ShippedFromDetails[5]:='Manufactured in the U.S. A  Plymouth, WI-USA';
                ShippedFromDetails[6]:='';
                ShippedFromDetails[7]:='H.S Code';
                ShippedFromDetails[8]:='8438-90-0000';
                ShippedFromDetails[9]:='';
                ...
                if not ShippingAgentRecGbl.get("Shipping Agent Code")then ShippingAgentRecGbl.Init();
                ...
                if("Order No." = '') and "Prepayment Invoice" then "Order No.":="Prepayment Order No.";
            end;
        }
    }
    requestpage
    ...
    begin
        ...
        if PrintCompany then begin
            ...
            CompanyAddress[8]:='The Netherlands';
    ...
    ShipDateCaptionLbl: Label 'Shipment Date';
    DueDateCaptionLbl: Label 'Due Date';
    TermsCaptionLbl: Label 'Payment Terms';
    CustomerIDCaptionLbl: Label 'Customer ID';
    PONumberCaptionLbl: Label 'Customer PO No.';
    ...
    TotalPriceCaptionLbl: Label 'Line Amount';
    ...
    ShippedFromDetails: array[9]of Text[100];
    BillToCustomerNoCaptionLbl: Label 'Bill-to Customer No.';
    ShippingAgentcodeCaptionLbl: Label 'Shipping Agent Code';
    CustomerVATRegNoCaptionLbl: Label 'Customer VAT Registration No.';
    EORINoCaptionLbl: Label 'EORI No.';
    ShippingAgentRecGbl: Record "Shipping Agent";
    HideLine: Boolean;
    NewLine: Boolean;
