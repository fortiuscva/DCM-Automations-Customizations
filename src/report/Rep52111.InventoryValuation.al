report 52111 "THK Inventory Valuation"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/THKInventoryValuation.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'THK Inventory Valuation';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            column(PPICode; Item."THK PPI Code")
            {
            IncludeCaption = true;
            }
            ...
            dataitem(BufferLoop; "Integer")
            {
                ...
                trigger OnAfterGetRecord()
                begin
                    if TempEntryBuffer.Next() <> 1 then CurrReport.Break();
                    if ExportToExcelVarGbl then begin
                        UnitCostVarGbl:=0;
                        if TempEntryBuffer."Remaining Quantity" <> 0 then UnitCostVarGbl:=TempEntryBuffer.Value1 / TempEntryBuffer."Remaining Quantity";
                        RowNo+=1;
                        EnterCell(RowNo, 1, Format(Item."No."), false, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                        EnterCell(RowNo, 2, Format(Item.Description), false, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                        EnterCell(RowNo, 3, Format(Item."THK PPI Code"), false, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                        EnterCell(RowNo, 4, Format(Item."Base Unit of Measure"), false, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                        EnterCell(RowNo, 5, Format(Item."Costing Method"), false, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                        EnterCell(RowNo, 6, format(TempEntryBuffer."Remaining Quantity"), false, false, false, false, '', TempExcelBuffer."Cell Type"::Number);
                        EnterCell(RowNo, 7, Format(UnitCostVarGbl), false, false, false, false, '', TempExcelBuffer."Cell Type"::Number);
                        EnterCell(RowNo, 8, Format(TempEntryBuffer.Value1), false, false, false, false, '', TempExcelBuffer."Cell Type"::Number);
                    end;
                end;
                trigger OnPreDataItem()
                begin
                    ...
                    if Item."Location Filter" <> '' then TempEntryBuffer.SetFilter("Location Code", '%1', Item."Location Filter");
                    if Item."Variant Filter" <> '' then TempEntryBuffer.SetFilter("Variant Code", '%1', Item."Variant Filter");
                end;
            }
            trigger OnAfterGetRecord()
            begin
            ...
            begin
                ...
                if ExportToExcelVarGbl then begin
                    ...
                    RowNo:=1;
                    EnterCell(RowNo, 1, Text010VarLbl, true, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                    EnterCell(RowNo, 8, format(CurrentDateTime), true, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                    RowNo+=1;
                    EnterCell(RowNo, 1, CompanyName, true, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                    //EnterCell(RowNo, 8, format(CurrentDateTime), true, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                    RowNo+=1;
                    EnterCell(RowNo, 1, StrSubstNo(Text003, AsOfDate), true, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                    EnterCell(RowNo, 8, format(UserId), true, false, false, false, '', TempExcelBuffer."Cell Type"::Text);
                    RowNo+=2;
                    EnterCell(RowNo, 1, 'No.', true, false, true, false, '', TempExcelBuffer."Cell Type"::Text);
                    EnterCell(RowNo, 2, 'Description', true, false, true, false, '', TempExcelBuffer."Cell Type"::Text);
                    EnterCell(RowNo, 3, 'PPI Code', true, false, true, false, '', TempExcelBuffer."Cell Type"::Text);
                    EnterCell(RowNo, 4, 'Base Unit of Measure', true, false, true, false, '', TempExcelBuffer."Cell Type"::Text);
                    EnterCell(RowNo, 5, 'Costing Method', true, false, true, false, '', TempExcelBuffer."Cell Type"::Text);
                    EnterCell(RowNo, 6, 'Remaining Qty.', true, false, true, false, '', TempExcelBuffer."Cell Type"::Text);
                    EnterCell(RowNo, 7, 'Unit Cost', true, false, true, false, '', TempExcelBuffer."Cell Type"::Text);
                    EnterCell(RowNo, 8, 'Inventory Value', true, false, true, false, '', TempExcelBuffer."Cell Type"::Text);
                end;
            end;
        }
    }
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(ExportToExcelVarGbl; ExportToExcelVarGbl)
                    {
                        ApplicationArea = All;
    ...
    begin
        Progress.Close();
        if ExportToExcelVarGbl then begin
            TempExcelBuffer.CreateNewBook(Text009VarLbl);
            TempExcelBuffer.WriteSheet(Text010VarLbl, CompanyName, UserId);
    ...
    ExportToExcelVarGbl: Boolean;
    TempExcelBuffer: Record "Excel Buffer" temporary;
    Text009VarLbl: Label 'Data';
    Text010VarLbl: Label 'Inventory Valuation';
    RowNo: Integer;
    UnitCostVarGbl: Decimal;
    ...
    begin
        with ItemLedgEntry do begin
            ...
            if Positive then begin
                ...
                ItemApplnEntry.SetCurrentKey("Inbound Item Entry No.", "Item Ledger Entry No.", "Outbound Item Entry No.", "Cost Application");
            ...
            begin
                ItemApplnEntry.Reset();
                ItemApplnEntry.SetCurrentKey("Outbound Item Entry No.", "Item Ledger Entry No.", "Cost Application", "Transferred-from Entry No.");
                ItemApplnEntry.SetRange("Item Ledger Entry No.", "Entry No.");
                ItemApplnEntry.SetRange("Outbound Item Entry No.", "Entry No.");
                ItemApplnEntry.SetRange("Posting Date", 0D, AsOfDate);
                if ItemApplnEntry.Find('-')then repeat if ItemLedgEntry2.Get(ItemApplnEntry."Inbound Item Entry No.") and (ItemLedgEntry2."Posting Date" <= AsOfDate)then "Remaining Quantity":="Remaining Quantity" - ItemApplnEntry.Quantity;
            ...
            ValueEntry.CalcSums("Cost Amount (Expected)", "Cost Amount (Actual)", "Cost Amount (Expected) (ACY)", "Cost Amount (Actual) (ACY)");
            "Cost Amount (Actual)":=Round(ValueEntry."Cost Amount (Actual)" + ValueEntry."Cost Amount (Expected)");
            "Cost Amount (Actual) (ACY)":=Round(ValueEntry."Cost Amount (Actual) (ACY)" + ValueEntry."Cost Amount (Expected) (ACY)", Currency."Amount Rounding Precision");
        end;
    end;
    ...
    begin
        ...
        if NewRow then UpdateTempEntryBuffer();
