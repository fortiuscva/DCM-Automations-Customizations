report 52113 "THK Aged Accounts Payable NA"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/THKAgedAccountsPayableNA.rdl';
    ApplicationArea = All;
    Caption = 'THK Aged Accounts Payable with Expense GL';
            ...
            dataitem(Vendor; Vendor)
            {
                ...
                dataitem("Vendor Ledger Entry"; "Vendor Ledger Entry")
                {
                    ...
                    begin
                        ...
                        if "Remaining Amount" <> 0 then InsertTemp("Vendor Ledger Entry");
                ...
                dataitem(Totals; "Integer")
                {
                    ...
                    Var
                        ...
                        GLAccountNo: Code[20];
                        NoOfIncomeStatementAccounts: Integer;
                        Is52200Account: Boolean;
                        GLDescription: Text[100];
                    begin
                        CalcPercents(AmountDueToPrint, AmountDue);
                        Clear(AmountDue);
                        AmountDueToPrint:=0;
                        if Number = 1 then TempVendLedgEntry.Find('-')
                        else
                        ...
                        if TempVendLedgEntry."Remaining Amount" = 0 then CurrReport.Skip();
                        if TempVendLedgEntry."Currency Code" <> '' then TempVendLedgEntry."Remaining Amt. (LCY)":=Round(CurrExchRate.ExchangeAmtFCYToFCY(PeriodEndingDate[1], TempVendLedgEntry."Currency Code", '', TempVendLedgEntry."Remaining Amount"));
                        if PrintAmountsInLocal then begin
                            TempVendLedgEntry."Remaining Amount":=Round(CurrExchRate.ExchangeAmtFCYToFCY(PeriodEndingDate[1], TempVendLedgEntry."Currency Code", Vendor."Currency Code", TempVendLedgEntry."Remaining Amount"), Currency."Amount Rounding Precision");
                            AmountDueToPrint:=TempVendLedgEntry."Remaining Amount";
                        end
                        else
                            AmountDueToPrint:=TempVendLedgEntry."Remaining Amt. (LCY)";
                        case AgingMethod of AgingMethod::"Due Date": AgingDate:=TempVendLedgEntry."Due Date";
                        AgingMethod::"Trans Date": AgingDate:=TempVendLedgEntry."Posting Date";
                        AgingMethod::"Document Date": AgingDate:=TempVendLedgEntry."Document Date";
                        end;
                        j:=0;
                        while AgingDate < PeriodEndingDate[j + 1]do j:=j + 1;
                        if j = 0 then j:=1;
                        ...
                        if UseExternalDocNo then DocNo:="Vendor Ledger Entry"."External Document No."
                        else
                            DocNo:="Vendor Ledger Entry"."Document No.";
                        TotalNumberOfEntries-=1;
                        if TotalNumberOfEntries = 0 then begin
                            for j:=1 to 4 do GrandBalanceDue[j]+="BalanceDue$"[j];
                            GrandTotalBalanceDue+="TotalBalanceDue$";
                        end;
                        clear(ExpenseAcc);
                        clear(ExpenseAccDesc);
                        clear(StopLoopGbl);
                        ExpenseAcc:='inventory';
                        ExpenseAccDesc:='';
                        GLEntryRecGbl.reset();
                        GLEntryRecGbl.Setrange("Document Type", "Vendor Ledger Entry"."Document Type");
                        GLEntryRecGbl.Setrange("Document No.", "Vendor Ledger Entry"."Document No.");
                        if GLEntryRecGbl.FindSet()then begin
                            repeat if GLAccountRec.GET(GLEntryRecGbl."G/L Account No.")then begin
                                    if GLAccountRec."Income/Balance" = GLAccountRec."Income/Balance"::"Income Statement" then begin
                                        if GLAccountRec."No." <> '52200' then begin
                                            ExpenseAcc:=GLAccountRec."No.";
                                            ExpenseAccDesc:=GLAccountRec.Name;
                                            StopLoopGbl:=true;
                                        end;
                                    end;
                                end;
                            until(GLEntryRecGbl.Next() = 0) or StopLoopGbl;
                        end;
                    ...
                    begin
                        if TempVendLedgEntry.Count() > 0 then begin
                            for j:=1 to 4 do AmountDue[j]:=VendTotAmountDue[j];
                ...
                begin
                    ...
                    if "Privacy Blocked" then BlockedDescription:=PrivacyBlockedTxt
                    else
                        BlockedDescription:='';
                    if Blocked <> Blocked::" " then BlockedDescription:=StrSubstNo(VendorBlockedLbl, Blocked)
                    else
                        BlockedDescription:='';
                    ...
                    if ShowAllForOverdue and (Format(ShowOnlyOverDueBy) <> '')then begin
                        ...
                        if VendLedgEntry.IsEmpty()then CurrReport.Skip();
                    end;
                end;
                trigger OnPreDataItem()
                begin
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(LengthOfAgingPeriods; PeriodCalculation)
                    {
                        ...
                        trigger OnValidate()
                        begin
                            if Format(PeriodCalculation) = '' then Error(PeriodCalculationRequiredLbl);
                        end;
                    }
                    field(ShowOnlyOverDueByControl; ShowOnlyOverDueBy)
                    {
                        ...
                        trigger OnValidate()
                        begin
                            if AgingMethod <> AgingMethod::"Due Date" then Error(OnlyForDueDateLbl);
                    ...
                    field(ShowAllForOverdueControl; ShowAllForOverdue)
                    {
                        ...
                        begin
                            if AgingMethod <> AgingMethod::"Due Date" then Error(OnlyForDueDateLbl);
                            if ShowAllForOverdue and (Format(ShowOnlyOverDueBy) = '')then Error(ShowOnlyOverdueLbl);
                        end;
                    }
    ...
    GLEntryRecGbl: record "G/L Entry";
    ...
    ExpenseAccountLbl: Label 'Expense Account';
    ExpenseAccountDescriptionLbl: Label 'Expense Account Description';
    ...
    begin
        with TempVendLedgEntry do begin
            if Get(VendLedgEntry."Entry No.")then exit;
            TempVendLedgEntry:=VendLedgEntry;
            case AgingMethod of AgingMethod::"Due Date": "Posting Date":="Due Date";
            AgingMethod::"Document Date": "Posting Date":="Document Date";
            end;
            Insert();
        end;
    end;
    ...
    begin
        Clear(PercentString);
        if Total <> 0 then for i:=1 to 4 do begin
                Percent:=Amounts[i] / Total * 100.0;
                if StrLen(Format(Round(Percent))) + 4 > MaxStrLen(PercentString[1])then PercentString[i]:=PadStr(PercentString[i], MaxStrLen(PercentString[i]), '*')
                else
                begin
                    PercentString[i]:=Format(Round(Percent));
                    k:=StrPos(PercentString[i], '.');
                    if k = 0 then PercentString[i]:=PercentString[i] + '.00'
                    else if k = StrLen(PercentString[i]) - 1 then PercentString[i]:=PercentString[i] + '0';
    ...
    begin
        ...
        else if Currency.Code <> CurrencyCode then Currency.Get(CurrencyCode);
    end;
    local procedure GetCurrencyCaptionCode(CurrencyCode: Code[10]): Text[80]begin
        if PrintAmountsInLocal then begin
            if CurrencyCode = '' then exit('101,1,' + AmountsAreInLbl);
    ...
    begin
        ...
        ExcelBuf.AddInfoColumn(REPORT::"THK Aged Accounts Payable NA", false, false, false, false, '', ExcelBuf."Cell Type"::Number);
        ExcelBuf.NewRow();
        ...
        if PrintAmountsInLocal then ExcelBuf.AddInfoColumn(Format(AsindicatedinDataLbl), false, false, false, false, '', ExcelBuf."Cell Type"::Text)
        else
    ...
    begin
        ...
        if PrintAmountsInLocal then if PrintDetail then ExcelBuf.AddColumn(Format(DocumentCurrencyLbl), false, '', true, false, true, '', ExcelBuf."Cell Type"::Text)
            else
                ExcelBuf.AddColumn(Format(VendorCurrency2Lbl), false, '', true, false, true, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn(ExpenseAccountLbl, false, '', true, false, true, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn(ExpenseAccountDescriptionLbl, false, '', true, false, true, '', ExcelBuf."Cell Type"::Text);
    end;
    ...
    begin
        ...
        if PrintAmountsInLocal then begin
            if PrintDetail then CurrencyCodeToPrint:="Vendor Ledger Entry"."Currency Code"
            else
                CurrencyCodeToPrint:=Vendor."Currency Code";
            if CurrencyCodeToPrint = '' then CurrencyCodeToPrint:=GLSetup."LCY Code";
            ExcelBuf.AddColumn(CurrencyCodeToPrint, false, '', false, false, false, '', ExcelBuf."Cell Type"::Text)end;
        ExcelBuf.AddColumn(ExpenseAcc, false, '', false, false, false, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn(ExpenseAccDesc, false, '', false, false, false, '', ExcelBuf."Cell Type"::Text);
    end;
    local procedure CreateExcelbook()
    begin
        //ExcelBuf.CreateBookAndOpenExcel('', DataLbl, AgedAccountsPayableLbl, CompanyName(), UserId());
        ExcelBuf.CreateNewBook(DataLbl);
        ExcelBuf.WriteSheet(AgedAccountsPayableLbl, CompanyName(), UserId());
        ExcelBuf.CloseBook();
        ExcelBuf.OpenExcel();
    end;
}
