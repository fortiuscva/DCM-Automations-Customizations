report 52109 "THK Aged Acc. Receivable NA"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/THKAgedAccountReceivableNA.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'THK Aged Accounts Receivable';
            ...
            dataitem(Customer; Customer)
            {
                ...
                dataitem(Totals; "Integer")
                {
                    ...
                    begin
                        ...
                        AgingMethod::"Document Date": AgingDate:=TempCustLedgEntry."Document Date";
                        end;
                        j:=0;
                ...
                begin
                    if not TempCustomer.get("No.")then CurrReport.Skip();
                    Clear("BalanceDue$");
                    if PrintAmountsInLocal then begin
                    ...
                    if "Privacy Blocked" then BlockedDescription:=PrivacyBlockedTxt
                        ...
                        BlockedDescription:='';
                    if Blocked <> Blocked::" " then BlockedDescription:=StrSubstNo(CustomerBlockedLbl, Blocked)
                    else
                        BlockedDescription:='';
                    ...
                    begin
                        ...
                        if "Net Change (LCY)" > "Credit Limit (LCY)" then OverLimitDescription:=OverLimitLbl
                        else
                            OverLimitDescription:='';
                    ...
                    if not TempCustLedgEntry.IsEmpty()then TempCustLedgEntry.DeleteAll();
                    ...
                    if ShowAllForOverdue and (Format(ShowOnlyOverDueBy) <> '')then begin
                        ...
                        if CustLedgEntry.IsEmpty()then CurrReport.Skip();
                    end;
                end;
                trigger OnPreDataItem()
                begin
                    NumCustLedgEntriesperCust.SetFilter(Customer_No, GetFilter("No."));
                    if NumCustLedgEntriesperCust.Open()then while NumCustLedgEntriesperCust.Read()do if not TempCustomer.get(NumCustLedgEntriesperCust.Customer_No)then begin
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(ShowAllForOverdueControl; ShowAllForOverdue)
                    {
                        ...
                        begin
                            if AgingMethod <> AgingMethod::"Due Date" then Error(OnlyForDueDateLbl);
                            if ShowAllForOverdue and (Format(ShowOnlyOverDueBy) = '')then Error(ShowOnlyOverdueByLbl);
                        end;
                    }
    ...
    protected var NumberOfLines: Integer;
    ...
    begin
        ...
        //Adding SalesPersonCode
        ExcelBuf.AddColumn(Customer.FieldCaption("Salesperson Code"), false, '', true, false, true, '', ExcelBuf."Cell Type"::Text);
    ...
    begin
        ...
        //Adding SalesPersonCode
        ExcelBuf.AddColumn("Cust. Ledger Entry"."Salesperson Code", false, '', false, false, false, '', ExcelBuf."Cell Type"::Text);
        if PrintDetail then begin
        ...
        else if OverLimitDescription = '' then ExcelBuf.AddColumn(CreditLimitToPrint, false, '', false, false, false, '#,##0', ExcelBuf."Cell Type"::Number)
            else
        ...
        if PrintAmountsInLocal then begin
            if PrintDetail then CurrencyCodeToPrint:="Cust. Ledger Entry"."Currency Code"
            else
